...
class DateParser(object):
    ...
    def parse(self, text, base=None, shift_dates=True):
        ...
        try:
            ...
            all_info = datetime.datetime.strptime(
                text, ACCEPTED_DAY_TIME_FORMAT)
        except ValueError:
            pass
        else:
    ...
    def determine_day(self, day_token, shift_dates):
        ...
        if day_token == 'tomorrow':
            ...
            date, valid_format = self._extract_time(
                day_token, ACCEPTED_DAY_FORMATS)
            if not date:
                LOG.debug('%s did not match any expected day formats' %
                          day_token)
                return None
...
class DateRange(object):
    ...
    def to_when(self):
        ...
        if not self.start or not self.end:
            raise Error(
                'Cannot convert range of dates without start and end points.')
        else:
            start = self.start
...
class DateRangeParser(object):
    ...
    def parse(self, date_string, shift_dates=False):
        ...
        if start_text:
            start_date = self.date_parser.parse(
                start_text, shift_dates=shift_dates)
