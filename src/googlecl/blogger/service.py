...
class BloggerServiceCL(gdata.blogger.service.BloggerService,
    ...
    def __init__(self, config):
        """Constructor."""
        gdata.blogger.service.BloggerService.__init__(
    ...
    def _get_blog_id(self, blog_title=None, user_id='default'):
        ...
        """
        blog_entry = self.GetSingleEntry(
            '/feeds/' + user_id + '/blogs', blog_title)
        if blog_entry:
            return blog_entry.GetSelfLink().href.split('/')[-1]
        else:
            if blog_title is not None:
                LOG.error(
                    'Did not find a blog with title matching %s', blog_title)
            else:
    ...
    def label_posts(self, post_entries, tags):
        ...
        for post in post_entries:
            ...
            if remove_set and not replace_tags:
                ...
                # 2) Are of the same scheme, but the term is in the 'remove'
                # set
            ...
            if replace_tags:
                ...
                post.category = [
                    c for c in post.category if c.scheme != scheme]
            if add_set:
                new_tags = [atom.Category(term=tag, scheme=scheme)
                            for tag in add_set]
