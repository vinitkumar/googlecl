...
class DocManager():
    ...
    def run(self, argv, isHelp, verbose):
        ...
        if servicename + '.' + version in self.apis:
            ...
            resp, content = http.request(SERVICE_URL % (
                self.base_url, servicename, version))
        ...
        if p in base:
            ...
            while (n == 1 or len(base) == 1) and not p in base:
                ...
                if 'resources' in base:
                    i = base.keys().index('resources')
                elif 'methods' in base:
                    i = base.keys().index('methods')
                elif 'parameters' in base:
                    i = base.keys().index('parameters')
                base = base[base.keys()[i]]
                n = ('resources' in base) + \
                    ('methods' in base) + ('parameters' in base)
            if p in base:
                base = base[p]
            else:
