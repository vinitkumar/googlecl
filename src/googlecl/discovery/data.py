...
class DefaultManager:
    ...
    def __init__(self, email):
        ...
        self.local_apis = config.lazy_get(
            None, 'local_apis', default='', option_type=str)
        if '[' in self.local_apis or '(' in self.local_apis:
            self.local_apis = json.loads(self.local_apis)
        self.base_url = config.lazy_get(
            None, 'base_url', default='https://www.googleapis.com', option_type=str)
    ...
    def fill_out_options(self, metinfo, doc, args):
        ...
        if not kwargs['prompt'] == 'none' and 'body' not in kwargs and 'request' in metinfo:
            ...
            if kwargs['editmode'] == 'editor':
                ...
                # Will modify to use required/mutable args if/when it is
                # available
