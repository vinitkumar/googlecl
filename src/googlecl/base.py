    ...
    def delete_entry_list(self, entries, entry_type, prompt,
        ...
        for item in entries:
            ...
            if delete:
                try:
                    if callback:
                        # if callback is provided then deletion is done by
                        # calling it
                        callback(item)
                    else:
                        # Later versions are defined with lowercase function names.
                        # These versions take GDataEntry objects, older takes
                        # the edit link.
        ...
        while try_forever or attempts_remaining:
            try:
                ...
                try:
                    ...
                    # RequestError defined in gdata.service (and raised by
                    # GDataService)
            ...
            for entry in entry_list:
                if self.label_delimiter is not None:
                    label = self._extract_label(
                        entry, label_attr=label_attribute)
                    if label:
    ...
    for attr in attribute_list:
        try:
            ...
            try:
                ...
                val = getattr(wrapped_entry.entry,
                              attr).text or missing_field_value
            except AttributeError:
                LOG.debug(err.args[0] + ' (value for field ' + attr + ')')
